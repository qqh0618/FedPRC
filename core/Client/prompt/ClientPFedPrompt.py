...
class ClientPFedPrompt(Client):
    ...
    def __init__(self, args, model, local_idx_dataidx_map, idx, logger, code_length, num_classes, device):
        ...
        # 创建一个[10,512]的矩阵
        self.Mk = torch.zeros(self.args.num_classes, 512).to(self.device)
        self.Mv = torch.zeros(self.args.num_classes, self.args.num_classes).to(self.device)
    def update_weights(self, global_round):  # 训练模型
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (X, y) in enumerate(self.trainloader):
                ...
                output, image_features, text_features = self.model(X)
                Fs = image_features.reshape(image_features.shape[0],image_features.shape[1])
                # 计算 Mk 和 Mv
                # And as for Mv, we use  the corresponding ground-truth label Ltrain after one-hot operation as our values in Mv.
                for i in range(self.args.num_classes):
                    self.Mk[i] += Fs[y == i].mean(0)

                A = torch.cosine_similarity(Fs, self.Mk[y])

                F_personal = A*y

                p_loss = (0.5*F_personal).mean()+1e-3

                loss = self.ce(output, y)+p_loss
                loss.backward()
