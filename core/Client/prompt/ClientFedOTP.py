import itertools
import time

import torch
import torch.nn as nn
import torch.optim as optim
from core.Client.ClientBase import Client
from utils import Accuracy
import torch.nn.functional as F



class ClientFedOTP(Client):
    """
    This class is for train the local model with input global model(copied) and output the updated weight
    args: argument
    Loader_train,Loader_val,Loaders_test: input for training and inference
    user: the index of local model
    idxs: the index for data of this local model
    logger: log the loss and the process
    """

    def __init__(self, args, model, local_idx_dataidx_map, idx, logger, code_length, num_classes, device):
        super().__init__(args, model, local_idx_dataidx_map, idx, logger, code_length, num_classes, device)
        self.N = 1
        self.use_uniform = True
        self.eps = 1
        self.thresh = 1e-2
        self.OT = 'sinkhorn'
        self.top_percent = 0.8
        self.max_iter = 10
        if self.args.dataset=='pacs':
            self.n_cls = 7
        elif self.args.dataset=='caltech':
            self.n_cls = 10
        elif self.args.dataset=='vlsc':
            self.n_cls = 5
        elif self.args.dataset=='office_home':
            self.n_cls = 61

    def update_weights(self, global_round):  # 训练模型
        self.model.to(self.device)
        self.model.train()
        epoch_loss = []
        optimizer = optim.SGD(itertools.chain(
                self.model.prompt_learner.parameters()
    ), lr=self.args.lr, momentum=0.9, weight_decay=1e-5)

        # optimizer = optim.SGD(itertools.chain(
        #     self.model.img_proj.parameters(),
        #     self.model.txt_proj.parameters()
        # ), lr=self.args.lr, momentum=0.9, weight_decay=1e-5)
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=self.args.lr_sh_rate, gamma=0.5)
        self.trainloader = self.get_trainloader()
        # self.print_model()
        for iter in range(self.args.local_ep):
            batch_loss = []
            start_time = time.time()
            for batch_idx, (X, y) in enumerate(self.trainloader):
                X = X.to(self.device)
                y = y.to(self.device)
                # 展示图片

                # 转为半精度
                # X = X.half()
                # y = y.half()

                output, img_f, text_f = self.model.forward_otp(X)

                loss = self.ce(output, y)
                loss.backward()
                if self.args.clip_grad != None:
                    nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=self.args.clip_grad)
                optimizer.zero_grad()
                optimizer.step()
                batch_loss.append(loss.item())
            # print("一轮训练耗时:", time.time() - start_time, )
            total_loss = sum(batch_loss) / len(batch_loss)
            epoch_loss.append(total_loss)
            self.logger.info(f"Round {global_round} Client {self.idx} Epoch {iter} Loss {total_loss}")
        self.save_model(self.args.logdir + '/client_' + str(self.idx) + '.pth')
        return self.model.state_dict(), sum(epoch_loss) / len(epoch_loss)

    def load_model(self, global_weights):
        self.model.load_state_dict(global_weights, strict=False)

    def test_accuracy(self):
        self.model.eval()
        accuracy = 0
        cnt = 0

        for X, y in self.testloader:
            X = X.to(self.device)
            # X = X.half()
            y = y.to(self.device)
            p, _, _ = self.model.forward_otp(X)
            y_pred = p.argmax(1)
            accuracy += Accuracy(y, y_pred)
            cnt += 1
        return accuracy / cnt