...
from core.Client.dg.ClientFedLGF import ClientFedLGF
...
# 2024 https://github.com/cnyanhao/FedLGF   renamed as FedSVD

class ServerFedLGF(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedLGF(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
