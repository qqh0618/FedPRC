...
from core.Client.dg.ClientFedAlign import ClientFedAlign
...
class ServerFedAlign(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedAlign(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))

    def generate_statistic_pool(self):
        # generate style pool
        statistic_pool = {"mean": [], "std": []}
        for client_id in range(self.args.num_clients):
            local_statistic_pool = self.LocalModels[client_id].compute_statistic()
        ...
        for epoch in tqdm(range(self.args.comm_round)):
            ...
            for idx in idxs_users:
                if self.args.upload_model == True:
                    ...
                    self.LocalModels[idx].set_statistic(copy.deepcopy(statistic_pool))
