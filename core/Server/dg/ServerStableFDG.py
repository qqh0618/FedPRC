...
from core.Client.dg.ClientStableFDG import ClientStableFDG
...
class ServerStableFDG(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientStableFDG(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.args.comm_round)):
            ...
            local_style = []
            for idx in range(self.args.num_clients):
                style_device = self.LocalModels[idx].style_collect()
                local_style.append(style_device)

            k = 0
            new_index = idxs_users

            # while True:
            #     for sa in range(len(idxs_users)):
            #         try:
            #             sample = random.sample(set(idxs_users) - (set([idxs_users[sa]]) | set(new_index)), 1)[0]
            #             new_index.append(sample)
            #         except:
            #             pass
            #
            #     if len(new_index) == len(idxs_users):
            #         break
            # # =================
            ...
            for idx in idxs_users:
                if self.args.upload_model == True:
                    ...
                    if self.style_buffer!=0:
                        self.LocalModels[idx].load_style(self.style_buffer[new_index[k]])
            ...
            # ==================
            # StableFDG
            self.style_buffer = copy.deepcopy(local_style)
            # ==================
            # print loss
