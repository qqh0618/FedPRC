...
from core.Client.dg.ClientFedOMG import ClientFedOMG
...
class ServerFedOMG(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedOMG(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.args.comm_round)):
            ...
            self.csv_log({'DataName': f"{self.args.dataset}_{self.args.leave_domain}", 'Round': epoch+1, 'Loss': loss_avg, 'Personal_Acc': test_accuracy / len(idxs_users), 'Global_Acc': cur_g_acc})
    ...
    def FedOMG(self,global_model, global_lr, cagrad_c, idxs_users):
        ...
        # print("flatten global model",flatten_global_weights.size())
        # for domain_name in dataobj.train_domain_list:
        #     domain_grad_diff = [torch.flatten(grad_param - global_param) * weight_dict[domain_name] for
