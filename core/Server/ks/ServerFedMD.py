...
from core.Client.ks.ClientFedMD import ClientFedMD
from tqdm import tqdm
import numpy as np
from utils import average_weights
from mem_utils import MemReporter
import time
from datautil.sampling import LocalDataloaders
import gc

class ServerFedMD(Server):
    def __init__(self, args, global_model,Loader_train,Loaders_local_test,Loader_global_test, pub_test,logger,device, net_idx_dataidx_map):
    ...
    def Create_Clints(self):
        ...
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedMD(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], loader_pub = self.public_data, idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
