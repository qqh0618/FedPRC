...
from core.Client.pfl.ClientFedROD import ClientFedROD
...
class ServerFedROD(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedROD(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
    ...
    def global_test_accuracy(self):
        ...
        for batch_idx, (X, y) in enumerate(self.global_testloader):
            X = X.to(self.device)
            y = y.to(self.device)
            f, out = self.global_model(X)

            out_p = self.global_model.classifier(f)
            p = out + out_p
