...
from core.Client.pfl.ClientFedETF import ClientFedETF
...
class ServerFedETF(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedETF(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(2)):
            ...
            for idx in idxs_users:
                ...
                self.LocalModels[idx].etf_proto = True
                self.LocalModels[idx].balancedloss = False

                self.LocalModels[idx].model.load_proto(self.global_model.proto_classifier.proto)
                w, loss = self.LocalModels[idx].update_weights(global_round=epoch)
                local_losses.append(copy.deepcopy(loss))
                local_weights.append(copy.deepcopy(w))
                acc = self.LocalModels[idx].test_accuracy()
        ...
        for epoch in tqdm(range(2)):
            ...
            for idx in idxs_users:
                ...
                self.LocalModels[idx].model.proto_classifier.proto.requires_grad = False
                self.LocalModels[idx].linear_proto = True
                self.LocalModels[idx].model.load_proto(self.global_model.proto_classifier.proto)
                w, loss = self.LocalModels[idx].update_weights(global_round=epoch)
                local_losses.append(copy.deepcopy(loss))
                local_weights.append(copy.deepcopy(w))
    ...
    #         X = X.to(self.device)
    #         y = y.to(self.device)
    #         f, _ = self.global_model(X)
    #
    #         p = torch.matmul(f, self.global_model.proto_classifier.proto.to(self.device))
    #
    #         y_pred = p.argmax(1)
