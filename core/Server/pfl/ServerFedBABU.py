...
from core.Client.pfl.ClientFedBABU import ClientFedBABU
...
class ServerFedBABU(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedBABU(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1)):
            test_accuracy = 0
            local_weights, local_losses = [], []
            self.logger.info(f'Global Training Round Fintune: {epoch+1}')
            m = max(int(self.args.sampling_rate * self.args.num_clients), 1)
            idxs_users = np.random.choice(range(self.args.num_clients), m, replace=False)
            for idx in idxs_users:
                if self.args.upload_model == True:
                ...
                self.LocalModels[idx].finetune = True
                w, loss = self.LocalModels[idx].update_weights(global_round=epoch)
                local_losses.append(copy.deepcopy(loss))
                local_weights.append(copy.deepcopy(w))
                acc = self.LocalModels[idx].test_accuracy()
    ...
    def global_test_accuracy(self):
        ...
        for batch_idx, (X, y) in enumerate(self.global_testloader):
            ...
            norm_prototype = self.global_model.prototype
