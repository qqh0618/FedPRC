...
from core.Client.pfl.ClientFedPAC import ClientFedPAC
...
class ServerFedPAC(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedPAC(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.args.comm_round)):
            ...
            for idx in idxs_users:
                ...
                self.LocalModels[idx].global_protos = self.global_protos

                w, loss = self.LocalModels[idx].update_weights(global_round=epoch)


                self.local_protos.append(self.LocalModels[idx].local_proto)
                self.Hs.append(self.LocalModels[idx].h)
                self.Vars.append(self.LocalModels[idx].v)
