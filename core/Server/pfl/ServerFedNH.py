...
from core.Client.pfl.ClientFedNH import ClientFedNH
...
class ServerFedNH(Server):
    ...
    def Create_Clints(self):
        for idx in range(self.args.num_clients):
            self.LocalModels.append(ClientFedNH(self.args, copy.deepcopy(self.global_model),self.net_idx_dataidx_map[idx], idx=idx, logger=self.logger, code_length = self.args.code_len, num_classes = self.args.num_classes, device=self.device))
    ...
    def global_test_accuracy(self):
        ...
        for batch_idx, (X, y) in enumerate(self.global_testloader):
            ...
            prototype = self.global_model.prototype  # 多了一个
            ...
            p = self.global_model.scaling * logits
