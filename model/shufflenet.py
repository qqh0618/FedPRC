...
class ShuffleNetV2(nn.Module):
    def __init__(
        ...
        self.line_squeeze = nn.Linear(output_channels, output_channels//4)
        self.v_head = nn.Linear(output_channels//4, num_classes)
    def _forward_impl(self, x: Tensor, ns=None) -> Tensor:
        ...
        x = self.stage4(x)
        x = self.conv5(x)
        x2 = x.mean([2, 3])  # globalpool
        out = self.fc(x2)
        # feats = self.line_squeeze(x2)
