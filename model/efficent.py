...
class EfficientNet(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.head = nn.Linear(lastconv_output_channels, num_classes)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.zeros_(m.bias)
        self.line_squeeze = nn.Linear(lastconv_output_channels, lastconv_output_channels//4)
        self.v_head = nn.Linear(lastconv_output_channels//4, num_classes)
    def _forward_impl(self, x: Tensor,ns = None) -> Tensor:
        x = self.features(x)
        ...
        x = self.classifier(x)
        out = self.head(x)
        # feats = self.line_squeeze(x)
        # v_out = self.v_head(feats)
        return out

    def forward(self, x: Tensor, ns=None) -> Tensor:
